...
class PurePursuit:
    ...
    def pure_pursuit_steer_control(self, car_state: CarState) -> Tuple[float, int]:
        ...
        alpha = math.atan2(self.path[index].coordinate[1] - car_state.rear_y,
                           self.path[index].coordinate[0] - car_state.rear_x) - car_state.yaw
    ...
    def search_target_index(self, car_state: CarState) -> Tuple[int, float]:
        ...
        if self.old_nearest_point_index is None:
            ...
            dx = [car_state.rear_x - point.coordinate[0] for point in self.path]
            dy = [car_state.rear_y - point.coordinate[1] for point in self.path]
            ...
            distance_this_index = car_state.calc_distance(self.path[index].coordinate[0], self.path[index].coordinate[1])
            while True:
                # Get the distance from the cars current position to the next point after the nearest point
                distance_next_index = car_state.calc_distance(self.path[index + 1].coordinate[0],
                                                              self.path[index + 1].coordinate[1])
        ...
        while adjusted_look_forward > car_state.calc_distance(self.path[index].coordinate[0],
                                                              self.path[index].coordinate[1]):
